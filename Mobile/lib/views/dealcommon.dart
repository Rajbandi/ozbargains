...
class DealCommon
{
  ...
  DealCommon(BuildContext context, GlobalKey<ScaffoldState> scaffoldKey)
  {
    ...
    nonTitleStyle = currentTheme
        .textTheme
        .caption
        .merge(new TextStyle(color: Colors.grey));

    primaryTitle =
        currentTheme.primaryTextTheme.bodyText1.copyWith(color: currentTheme.primaryColor);
    highlightTitle =
        currentTheme.accentTextTheme.bodyText1.copyWith(color: currentTheme.accentColor);        
  }
    ...
    if (d.meta.upcomingDate > 0) {
     ...
     return Row(
                ...
                children: <Widget>[
                  Expanded(
                      flex: 1,
                      child: getMetaAuthor(d, authorImage)
  ...
  Widget getNetworkImage(String url, double width, double height)
  {
      ...
      try{
          if(url != null && url.length>0)
            image = Image.network(url, width: width, height: height,);
      } 
      catch(e)
      {
    ...
    tags.forEach((element) {
      ...
      row.children.add(Container(
          ...
          decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(3.0), color: currentTheme.dividerColor),
          child: Text(
            element,
            style: tagStyle,
          )));
      ...
      if(d.meta.labels != null && d.meta.labels.length>0)
      {
        ...
        if(label != null && label is String)
        {
          ...
          spans.add(WidgetSpan(
            child: Container(
          child: Text(lbl,
              style: highlightTitle.copyWith(color: Colors.white)),
                    ...
                    children: <Widget>[
                      InkWell(
                        child: Container(
                            ...
                            child: Icon( FontAwesomeIcons.copy, color: primaryColor,)
                      ...
                      InkWell(
                        child: Container(
                            padding: EdgeInsets.only(right: 20),
                            child: Icon(Icons.content_copy, color: accentColor,)
                              
                            ),
                        onTap: () => {copyToClipboard("Copied external deal link ", d.snapshot.goto)},
                      ),
                    ],
                  );
  }
  ...
  Widget getVotes(Deal d)
  {
    return Column(
                            ...
                            crossAxisAlignment: CrossAxisAlignment.center,
                            children: <Widget>[
                              getVote((d.vote.up ?? "0"), FontAwesomeIcons.thumbsUp, Color(0x88009900)),
                              getVote((d.vote.down ?? "0"), FontAwesomeIcons.thumbsDown, Color(0x88ff0000))
                            ],
                          );
  }
